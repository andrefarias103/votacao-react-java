...
export class AgendaService {
  ...
  constructor(
    ...
    private readonly serviceVotation: VotationService,
  ...
  public async createAgenda(
    userId: number,
    dataAgenda: CreateAgendaDto,
  ): Promise<CreateAgendaDto> {
    const categoriaId: number = Number(dataAgenda.categoriaId);
    const category = await this.repositoryCategory.findById({
    ...
    const session = await this.serviceSession.createSession({
      dataHoraInicio,
      dataHoraFim,
    });
  ...
  public async findAllAgendas(): Promise<ListAgendaDto[]> {
    const agendas: ListAgendaDto[] = await this.repositoryAgenda.findAll({
      include: { categoria: true, Sessao: true, votacao: true },
    });
    if (agendas === null) {
      throw new NotFoundException("Nenhuma agenda foi encontrada");
    }
    const agendasComVotos = await Promise.all(
      agendas.map(async (agenda) => {
        if (agenda.Sessao) {
          agenda.Sessao.dataHoraInicio = new Date(
            agenda.Sessao.dataHoraInicio,
          ).toLocaleString();
          agenda.Sessao.dataHoraFim = new Date(
            agenda.Sessao.dataHoraFim,
          ).toLocaleString();
        }
        const quantidadeVotos = await this.serviceVotation.getResultVotation(
          agenda.id,
        );
  ...
  ////
  public async findAgendasByCategory(
    categoryId: number,
  ): Promise<ListAgendaDto[]> {
    const category = await this.repositoryCategory.findById({
      where: { id: categoryId },
    });
    if (category === null) {
  ...
  ////
  public async findAgendasByStatus(
    statusAgenda: string,
  ): Promise<ListAgendaDto[]> {
    const agendas: ListAgendaDto[] = await this.repositoryAgenda.findAll({
      include: { categoria: true, Sessao: true, votacao: true },
      where: { Sessao: { status: statusAgenda } },
    });
    ...
    const agendasComVotos = await Promise.all(
      agendas.map(async (agenda) => {
        ...
        const {
          ...
          quantidadeVotosNao,
          resultado,
        } = await this.serviceVotation.getResultVotation(agenda.id);
        return {
          ...agenda,
          quantidadeVotos,
          quantidadeVotosSim,
  ...
  ////
  public async findAgendasByCategoryAndStatus(
    categoryId: number,
    statusAgenda: string,
  ): Promise<ListAgendaDto[]> {
    const category = await this.repositoryCategory.findById({
    ...
    const agendas: ListAgendaDto[] = await this.repositoryAgenda.findAll({
      include: { categoria: true, Sessao: true, votacao: true },
      where: { categoriaId: categoryId, Sessao: { status: statusAgenda } },
    });
  ...
  ////
  // public async findAgendasByStatus1(
  //   categoryId?: number,
  //   statusAgenda?: string,
  // ): Promise<ListAgendaDto[]> {
  ...
  //       throw new NotFoundException("Categoria não foi encontrada");
  //     }
  //     agendas = await this.repositoryAgenda.findAll({
  //       include: { categoria: true, Sessao: true, votacao: true },
  //       where: { categoriaId: categoryId, Sessao: { status: statusAgenda } },
  //     });
  //   } else {
  ...
  //       agendas = await this.repositoryAgenda.findAll({
  //         include: { categoria: true, Sessao: true, votacao: true },
  //         where: { Sessao: { status: StatusSessaoEnum.STATUS_INICIADA } },
  //       });
  //     }
  //   }
  ...
  public async findAgendaById(id: number): Promise<ListAgendaDto[]> {
    const agenda: ListAgendaDto[] = await this.repositoryAgenda.findById({
      where: { id },
    });
    if (!agenda) {
      return [];
  ...
  public async findAgendaByName(nome: string): Promise<ListAgendaDto[]> {
    const agenda: ListAgendaDto[] = await this.repositoryAgenda.findAll({
      where: { titulo: { contains: nome, mode: "insensitive" } },
    });
    if (!agenda) {
      return [];
    }
  ...
  public async updateAgenda(id: number, dataAgenda: CreateAgendaDto) {
    ...
    if (agenda.sessaoId) {
      const session = await this.serviceSession.updateSession(agenda.sessaoId, {
        dataHoraInicio,
        dataHoraFim,
      });
    }
